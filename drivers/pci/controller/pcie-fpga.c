...
static int fpga_init(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	{
		error = PTR_ERR(pchar_class);
		printk("Fpga: class_create(): Error.\n");
		return error;
	}
	dev = device_create(pchar_class, NULL, MKDEV(MAJNUM, 0), NULL, DRIVER_NAME);
	if (IS_ERR(dev))
	{
		error = PTR_ERR(dev);
		printk("Fpga: device_create(): Error.\n");
		return error;
	}
